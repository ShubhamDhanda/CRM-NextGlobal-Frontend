...
class updateTechnicalRequestDialog extends StatefulWidget {
  final Map<String, dynamic> mp;
  const updateTechnicalRequestDialog({Key? key, required this.mp}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _updateTechnicalRequestDialogState(mp: mp);
}
...
class _updateTechnicalRequestDialogState extends State<updateTechnicalRequestDialog> {
  ...
  final snackBar3 = const SnackBar(
    content: Text('Technical Request  Updated Successfully'),
  ...
  _updateTechnicalRequestDialogState({required this.mp}) {

    requestedBy.text = mp["requestedBy"];
    projectName.text = mp["projectName"];
    requestTo.text = mp["requestTo"];
    requestDate.text = mp["requestDate"];
    revision.text = mp["revision"];
    revisionReason.text = mp["revisionReason"];
    returnBy.text = mp["returnBy"];
    designReceived.text = mp["designReceived"];
  ...
  void postData() async {
    try {
      ...
      if (validate() == true) {
        dynamic res = await apiClient.updateTechnicalRequest(mp["trId"], employeeMap[requestedBy.text], projectsMap[projectName.text],departmentMap[requestTo.text], requestDate.text, revision.text, revisionReason.text,returnBy.text,  designReceived.text);
        if (res["success"] == true) {
  ...
  Widget build(context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Update Technical Request"),

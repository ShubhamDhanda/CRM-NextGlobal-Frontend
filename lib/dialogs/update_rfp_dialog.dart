...
class UpdateRfpDialog extends StatefulWidget{
  Map<String, dynamic> mp;
  UpdateRfpDialog({super.key, required this.mp});

  @override
  State<StatefulWidget> createState() => _UpdateRfpDialogState(mp : mp);
}

const List<String> actions = ["Go", "NoGo", "Review"];

class _UpdateRfpDialogState extends State<UpdateRfpDialog>{
  Map<String, dynamic> mp;
  ...
  final snackBar3 = const SnackBar(
    content: Text('RFP Updated Successfully'),
  ...
  _UpdateRfpDialogState({required this.mp}){
    city.text = mp["city"];
    cityId = mp["cityId"];
    ...
    projectManagerId = mp["projectManagerId"];
    bidDate.text = mp["bidDate"];
    startDate.text = mp["startDate"];
    submissionDate.text = mp["submissionDate"];
    projectName.text = mp["projectName"];
    rfpNumber.text = mp["rfpNumber"];
    amount.text = mp["amount"].toString();
  }
  ...
  void _postData() async {
    try{
      if(validate() == true){
        dynamic res = await apiClient.updateRFP(mp["rfpId"], departmentId.toString(), action.toString(), projectManagerId.toString(), bidDate.text, startDate.text, submissionDate.text, projectName.text, rfpNumber.text, cityId.toString(), amount.text);

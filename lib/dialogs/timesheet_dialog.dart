    ...
    dynamic res = await apiClient.getAllTimesheet();
      ...
      if(res?["success"] == true){
        for(var e in res["res"]) {
          ...
          apps.add(
              CalendarEvent(
                  startTime: DateFormat("yyyy-MM-dd HH:mm:ss").parse('${DateFormat("yyyy-MM-dd").format(DateTime.parse(e["Date"]).toLocal())} ${e["Start_Time"]}'),
                  endTime: DateFormat("yyyy-MM-dd HH:mm:ss").parse('${DateFormat("yyyy-MM-dd").format(DateTime.parse(e["Date"]).toLocal())} ${e["End_Time"]}'),
            ...
            GestureDetector(
              onTap: () {
                showGeneralDialog(
                    ...
                    pageBuilder: (context, animation, secondaryAnimation) => const TimeSheetNewDialog()).then((value) {
                  if(value! == "Changed"){
                    setState(() {
                      dataLoaded = false;
                    });
        ...
        onTap: (calendarTapDetails) {
          if(calendarTapDetails.appointments!.length != null){
            showGeneralDialog(
                ...
                pageBuilder: (context, animation, secondaryAnimation) => TimeSheetUpdateDialog( calendarEvent: calendarTapDetails.appointments![0],));
...
class CalendarEvent extends Appointment{
  CalendarEvent({required super.startTime, required super.endTime, super.subject, super.color, this.comments});
  String? comments;
}
